    ...
    <div class="col-lg-4 col-xl-4">
        <form action="{{ route('edit', $item->id) }}" method="POST" class="form-modals border rounded border-dark d-flex flex-column align-items-md-center p-4 mt-5">
            ...
            <div class="form-group col-12 d-flex align-items-center justify-content-between">
                ...
                <input class="form-control p-2 w-auto @error('name')is-invalid @enderror" type="text" name="name" required="" value="{{ $item->name }}">
            </div>
            <div class="form-group col-12 d-flex align-items-center justify-content-between">
                <label class="mb-0 ml-auto mr-1" for="">Телефон</label>
                <input class="form-control p-2 w-auto @error('phone')is-invalid @enderror" type="number" name="phone" value="{{ $item->phone }}">
            </div>
            <div class="form-group col-12 d-flex align-items-center justify-content-between">
                <label class="mb-0 ml-auto mr-1" for="">Email</label>
                <input class="form-control p-2 w-auto @error('email')is-invalid @enderror" type="email" name="email" value="{{ $item->email }}">
            </div>
